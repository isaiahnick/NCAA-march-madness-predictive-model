...
def calculate_percentiles(df: pd.DataFrame, filter_condition: str, confidence_level: float) -> Dict[str, float]:
    ...
    """
    if filter_condition == 'champions_only':
    ...
    return {
        "AdjOE_5th": filtered_df['AdjOE z-score'].quantile(confidence_level),
        "AdjDE_5th": filtered_df['AdjDE z-score'].quantile(confidence_level),
        "AdjEM_5th": filtered_df['AdjEM z-score'].quantile(confidence_level)
    }
...
def calculate_metric_correlations(historical_df: pd.DataFrame, thresholds: Dict[str, float]) -> Dict[str, float]:
    ...
    qualifying_teams = historical_df[
        (historical_df['AdjOE z-score'] > thresholds['AdjOE_5th']) &
        (historical_df['AdjDE z-score'] > thresholds['AdjDE_5th']) &
        (historical_df['AdjEM z-score'] > thresholds['AdjEM_5th'])
    ]
...
def calculate_similarity_score(team_metrics: pd.Series, thresholds: Dict[str, float], 
    ...
    oe_score = max(0, team_metrics['AdjOE z-score'] - thresholds['AdjOE_5th'])
    de_score = max(0, team_metrics['AdjDE z-score'] - thresholds['AdjDE_5th'])
    em_score = max(0, team_metrics['AdjEM z-score'] - thresholds['AdjEM_5th'])
...
def analyze_teams_meeting_criteria(df: pd.DataFrame, thresholds: Dict[str, float], 
    ...
    criteria = (
        (df['AdjOE z-score'] > thresholds['AdjOE_5th']) & 
        (df['AdjDE z-score'] > thresholds['AdjDE_5th']) & 
        (df['AdjEM z-score'] > thresholds['AdjEM_5th'])
    )
...
def main():
    ...
    Configuration:
        ...
        - Round definitions and confidence levels
    """
    ...
    ROUNDS = [
        ('champions_only', 'Champions', 0.05, 1),
    ...
    for round_id, round_name, confidence_level, target_sum in ROUNDS:
        ...
        print(f"Thresholds:")
        print(f"  AdjOE z-score > {thresholds['AdjOE_5th']:.2f}")
        print(f"  AdjDE z-score > {thresholds['AdjDE_5th']:.2f}")
        print(f"  AdjEM z-score > {thresholds['AdjEM_5th']:.2f}")
...
# Export results to a single text file
    with open('tournament_analysis_results.txt', 'w') as f:
        ...
        for round_name, results in round_results.items():
            ...
            # Write thresholds
            f.write("Thresholds:\n")
            f.write(f"  AdjOE z-score > {results['thresholds']['AdjOE_5th']:.2f}\n")
            f.write(f"  AdjDE z-score > {results['thresholds']['AdjDE_5th']:.2f}\n")
            f.write(f"  AdjEM z-score > {results['thresholds']['AdjEM_5th']:.2f}\n\n")
